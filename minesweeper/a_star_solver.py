...
class Node(dataobject):
    ...
    def __init__(self, state: tuple[bool | None], h: int):
        self.state = state
        self.h = h
...
def child_states(cnf: KB, parent_state: tuple[bool | None], exclude):
...
def a_star_search(kb: KB, init_model: dict[int, bool], exclude) -> bool:
    ...
    while True:
        ...
        for child_state in child_states(kb, node.state, exclude):
            ...
            heapq.heappush(frontier, Node(child_state, h))


def gen_state_test(kb: KB, model: dict[int, bool]):
    return tuple(model[var] if var in model else True for var in kb.vars)
